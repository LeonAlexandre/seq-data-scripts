...
tar='inference_data1000by20_04_assemble2' #name of output dir
...
echo "edit sigmoid"
python report.py --outdir=$foo/$tar --assembled=$foo/$tar/recons_edit_logsin.txt --trace=$foo/$bar/test.trace --labels=$foo/$bar/test.label --mode=edit --bias=sigmoid
echo "hamming sigmoid"
python report.py --outdir=$foo/$tar --assembled=$foo/$tar/recons_hamming_logsin.txt --trace=$foo/$bar/test.trace --labels=$foo/$bar/test.label --mode=hamming --bias=sigmoid

python ./assemble2.py --outdir=$foo/$tar --f_inference="$foo/$car" --fragnum=$fragnum --mode="both" --bias=tanh
echo "edit tanh"
python report.py --outdir=$foo/$tar --assembled=$foo/$tar/recons_edit_logsin.txt --trace=$foo/$bar/test.trace --labels=$foo/$bar/test.label --mode=edit --bias=tanh
echo "hamming tanh"
python report.py --outdir=$foo/$tar --assembled=$foo/$tar/recons_hamming_logsin.txt --trace=$foo/$bar/test.trace --labels=$foo/$bar/test.label --mode=hamming --bias=tanh
