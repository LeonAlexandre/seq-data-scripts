#Uses a kmers approach to the NMT problem
#we choose size 4 arbitrarily as it shouldn't be much of a problem.
...
def parse_arguments():
    ...
    parser.add_argument('--fragnum',dest="fragnum",help='Number of fragments per sequence (int)',type=int,default=1)
...
def kmersify(list_in):
    mers = 4
    merlist = []
    
    cuts = int(math.floor(len(list_in) / mers))
    
    for i in range(cuts):
        merlist.append(list_in[ i * mers: (i + 1)* mers])
    #last fragment
...
def generate_cut_strings(args):
    ...
    for i in range(int(numSeq*train_size)):
        ...
        if not '' in trace:
            label = kmersify(label)
            train_label = train_label + '\n' + label
            for i in range(num_traces):
                trace[i] = kmersify(trace[i])                
